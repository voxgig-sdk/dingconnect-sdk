# Dingconnect JavaScript SDK



The JavaScript SDK for the Dingconnect API — an entity-oriented client with full async/await support.
...
```js
const { DingconnectSDK } = require('@voxgig-sdk/dingconnect-js')
...
### List AccountLookup Records

```js
const account_lookups = await client.AccountLookup().list()
for (const account_lookup of account_lookups) {
  console.log(account_lookup)
}
```
...
dingconnect/
├── src/
│   ├── DingconnectSDK.js        # Main SDK class
