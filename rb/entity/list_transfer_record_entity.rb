...
class ListTransferRecordEntity
  ...
  def make
    opts = @_entopts.dup
    ListTransferRecordEntity.new(@_client, opts)
  end
  ...
  # @return [ListTransferRecord, Hash] the current ListTransferRecord data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of ListTransferRecord fields)
  def match_get
  ...
  #
  # @param reqdata [ListTransferRecordCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [ListTransferRecord, Hash] the created ListTransferRecord; raises DingconnectError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = DingconnectHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
