...
class ErrorCodeDescriptionEntity
  ...
  def make
    opts = @_entopts.dup
    ErrorCodeDescriptionEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = DingconnectHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [ErrorCodeDescription, Hash] the current ErrorCodeDescription data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = DingconnectHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of ErrorCodeDescription fields)
  def match_get
  ...
  #
  # @param reqmatch [ErrorCodeDescriptionListMatch, Hash, nil] match filter (any subset of
  #   ErrorCodeDescription fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<ErrorCodeDescription>, Array] the matching ErrorCodeDescription items; raises DingconnectError on failure
