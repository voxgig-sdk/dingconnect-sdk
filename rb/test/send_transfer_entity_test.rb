...
class SendTransferEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = send_transfer_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "send_transfer." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DINGCONNECT_TEST_SEND_TRANSFER_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    send_transfer_ref01_ent = client.SendTransfer(nil)
...
def send_transfer_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "send_transfer", "SendTransferTestData.json")
  ...
  entid_env_raw = ENV["DINGCONNECT_TEST_SEND_TRANSFER_ENTID"]
