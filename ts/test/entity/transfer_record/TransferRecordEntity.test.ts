...
describe('TransferRecordEntity', async () => {
  ...
  test('instance', async () => {
    ...
    const ent = testsdk.TransferRecord()
    assert(null != ent)
  })
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create']) {
      if (maybeSkipControl(t, 'entityOp', 'transfer_record.' + op, live)) return
    }
    ...
    // CREATE
    const transfer_record_ref01_ent = client.TransferRecord()
    let transfer_record_ref01_data = setup.data.new.transfer_record['transfer_record_ref01']

    transfer_record_ref01_data = await transfer_record_ref01_ent.create(transfer_record_ref01_data)
    assert(null != transfer_record_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/transfer_record/TransferRecordTestData.json')
  ...
  const idmapEnvVal = process.env['DINGCONNECT_TEST_TRANSFER_RECORD_ENTID']
  ...
  idmap = env['DINGCONNECT_TEST_TRANSFER_RECORD_ENTID']
