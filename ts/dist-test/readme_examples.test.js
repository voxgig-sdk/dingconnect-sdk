...
function isRunnable(code) {
    return (/new\s+DingconnectSDK\b/.test(code) ||
        /\bDingconnectSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+DingconnectSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/DingconnectSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
