...
// Dingconnect SDK ErrorCodeDescription entity
...
class ErrorCodeDescriptionEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "error_code_description";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new ErrorCodeDescriptionEntity($this->_client, $opts);
    }

    /**
     * @param ErrorCodeDescription|array $args ErrorCodeDescription data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = DingconnectHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return ErrorCodeDescription|array The current ErrorCodeDescription data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of ErrorCodeDescription fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = DingconnectHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of ErrorCodeDescription fields).
     */
    ...
    /**
     * List ErrorCodeDescription items matching the given filter.
     *
     * @param ErrorCodeDescriptionListMatch|array|null $reqmatch Match filter (any subset
     *   of ErrorCodeDescription fields) as an assoc-array; ErrorCodeDescriptionListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ErrorCodeDescription[]|array A list of ErrorCodeDescription items as assoc-arrays at
     *   the SDK boundary; throws DingconnectError on failure (item-5 convention).
     */
