...
class SendTransferEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = DingconnectSDK::test(null, null);
        $ent = $testsdk->SendTransfer(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = send_transfer_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "send_transfer." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set DINGCONNECT_TEST_SEND_TRANSFER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $send_transfer_ref01_ent = $client->SendTransfer(null);
...
function send_transfer_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/send_transfer/SendTransferTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["send_transfer01", "send_transfer02", "send_transfer03"] as $k) {
    ...
    $entid_env_raw = getenv("DINGCONNECT_TEST_SEND_TRANSFER_ENTID");
