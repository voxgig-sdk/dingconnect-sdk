...
class FtCtrl extends DingconnectControl
{
...
class FtClient
{
    ...
    private DingconnectUtility $utility;

    public function __construct(DingconnectUtility $utility, array $options)
    {
    ...
    public function get_utility(): DingconnectUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DingconnectContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DingconnectContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DingconnectContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DingconnectConfig::make_feature($name);
    ...
    private static function buildUrl(DingconnectSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DingconnectError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DingconnectResponse($fetched);
        }
    ...
    private function fail(DingconnectContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DingconnectContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
