...
module DingconnectUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = DingconnectResponse.new({})
    ...
    if fetch_err
      ...
      response = DingconnectResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DingconnectResponse.new(fetched)
    else
