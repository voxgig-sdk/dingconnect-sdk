...
boru-driven command-line client **and** interactive REPL for the Dingconnect
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dingconnect-cli)
make build

# 2. See usage (words, entities, env vars)
./dingconnect-cli --help

# 3. Provide credentials once, via the environment
export DINGCONNECT_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./dingconnect-cli list account_lookup
./dingconnect-cli list balance

# 5. Override the API base URL for a single call
DINGCONNECT_BASE=https://api.example.com ./dingconnect-cli list account_lookup

# 6. No arguments -> interactive REPL
./dingconnect-cli
dingconnect> list account_lookup
dingconnect> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dingconnect-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/dingconnect-cli list account_lookup
   ```
...
```sh
export DINGCONNECT_APIKEY=sk_live_xxx            # API key
export DINGCONNECT_BASE=https://api.example.com  # optional: override the API base URL
./dingconnect-cli list account_lookup
```
...
Run with no arguments to open a REPL (prompt `dingconnect>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `account_lookup`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/dingconnect-cli`. |
...
(`./dingconnect-cli <expr>`) and interactively (the REPL), and expressions compose the
