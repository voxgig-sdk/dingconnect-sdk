...
local EstimateEntity = {}
EstimateEntity.__index = EstimateEntity


function EstimateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EstimateEntity)
  self._name = "estimate"
  self._client = client
...
function EstimateEntity:get_name()
  return self._name
end


function EstimateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EstimateEntity.new(self._client, opts)
end


function EstimateEntity:data_set(args)
  if args ~= nil then
...
function EstimateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EstimateEntity:match_set(args)
  if args ~= nil then
...
function EstimateEntity:match_get()
...
function EstimateEntity:stream(action, args, callopts)
...
---@param reqdata EstimateCreateData
---@param ctrl? table
---@return Estimate
---@return string? err
function EstimateEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function EstimateEntity:_run_op(ctx, post_done)
  local utility = self._utility
