...
local SendTransferEntity = {}
SendTransferEntity.__index = SendTransferEntity


function SendTransferEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SendTransferEntity)
  self._name = "send_transfer"
  self._client = client
...
function SendTransferEntity:get_name()
  return self._name
end


function SendTransferEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SendTransferEntity.new(self._client, opts)
end
...
function SendTransferEntity:mark_deleted()
  self._deleted = true
end


function SendTransferEntity:deleted()
  return true == self._deleted
end


function SendTransferEntity:data_set(args)
  if args ~= nil then
...
function SendTransferEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SendTransferEntity:match_set(args)
  if args ~= nil then
...
function SendTransferEntity:match_get()
...
function SendTransferEntity:stream(action, args, callopts)
...
---@param reqdata SendTransferCreateData
---@param ctrl? table
---@return SendTransfer
---@return string? err
function SendTransferEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function SendTransferEntity:_run_op(ctx, post_done)
  local utility = self._utility
