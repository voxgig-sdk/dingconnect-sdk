...
local EstimatePriceEntity = {}
EstimatePriceEntity.__index = EstimatePriceEntity


function EstimatePriceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EstimatePriceEntity)
  self._name = "estimate_price"
  self._client = client
...
function EstimatePriceEntity:get_name()
  return self._name
end


function EstimatePriceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EstimatePriceEntity.new(self._client, opts)
end
...
function EstimatePriceEntity:mark_deleted()
  self._deleted = true
end


function EstimatePriceEntity:deleted()
  return true == self._deleted
end


function EstimatePriceEntity:data_set(args)
  if args ~= nil then
...
function EstimatePriceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EstimatePriceEntity:match_set(args)
  if args ~= nil then
...
function EstimatePriceEntity:match_get()
...
function EstimatePriceEntity:stream(action, args, callopts)
...
---@param reqdata EstimatePriceCreateData
---@param ctrl? table
---@return EstimatePrice
---@return string? err
function EstimatePriceEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function EstimatePriceEntity:_run_op(ctx, post_done)
  local utility = self._utility
