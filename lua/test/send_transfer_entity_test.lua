...
describe("SendTransferEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:SendTransfer(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = send_transfer_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "send_transfer." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set DINGCONNECT_TEST_SEND_TRANSFER_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local send_transfer_ref01_ent = client:SendTransfer(nil)
    local send_transfer_ref01_data = helpers.to_map(vs.getprop(
...
function send_transfer_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/send_transfer/SendTransferTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read send_transfer test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("DINGCONNECT_TEST_SEND_TRANSFER_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["DINGCONNECT_TEST_SEND_TRANSFER_ENTID"])
