...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dingconnect-mcp)
make build
...
claude mcp add --scope user dingconnect \
  -- /absolute/path/to/dingconnect-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./dingconnect-mcp -transport http -addr :8080
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dingconnect-mcp
   ```
...
```sh
./dingconnect-mcp -transport http -addr :18080 &
