// Command Dingconnect-mcp is an MCP server exposing the Dingconnect
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Dingconnect_list  — list records of an entity
//	Dingconnect_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: DINGCONNECT_APIKEY carries the API key and
	// DINGCONNECT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DINGCONNECT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DINGCONNECT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDingconnectSDK(opts)
