...
import (
	...
	omni "github.com/voxgig-sdk/dingconnect-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DingconnectError); is {
				out := map[string]any{
					"name":    "DingconnectError",
...
func omniCtx(arg any, client *sdk.DingconnectSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DingconnectSDK); is {
			client = live
