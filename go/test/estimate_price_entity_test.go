...
func TestEstimatePriceEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.EstimatePrice(nil)
		if ent == nil {
			t.Fatal("expected non-nil EstimatePriceEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := estimate_priceBasicSetup(nil)
		...
		for _, _op := range []string{"create"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "estimate_price." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set DINGCONNECT_TEST_ESTIMATE_PRICE_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		estimatePriceRef01Data = core.ToMapAny(entityData(estimatePriceRef01DataResult))
		if estimatePriceRef01Data == nil {
...
func estimate_priceBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "estimate_price", "EstimatePriceTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read estimate_price test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse estimate_price test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("DINGCONNECT_TEST_ESTIMATE_PRICE_ENTID")
