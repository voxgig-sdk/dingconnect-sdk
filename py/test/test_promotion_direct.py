...
class TestPromotionDirect:
    ...
    def test_should_direct_list_promotion(self):
        setup = _promotion_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-promotion", "live" if setup["live"] else "unit")
        if _skip:
