...
class TestListTransferRecordEntity:

    def test_should_create_instance(self):
        testsdk = DingconnectSDK.test(None, None)
        ent = testsdk.ListTransferRecord(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _list_transfer_record_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "list_transfer_record." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DINGCONNECT_TEST_LIST_TRANSFER_RECORD_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        list_transfer_record_ref01_ent = client.ListTransferRecord(None)
        ...
        list_transfer_record_ref01_data = helpers.to_map(runner.entity_data(list_transfer_record_ref01_ent.create(list_transfer_record_ref01_data, None)))
        assert list_transfer_record_ref01_data is not None



def _list_transfer_record_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/list_transfer_record/ListTransferRecordTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "DINGCONNECT_TEST_LIST_TRANSFER_RECORD_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("DINGCONNECT_TEST_LIST_TRANSFER_RECORD_ENTID"))
