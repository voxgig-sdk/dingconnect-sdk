...
class TestLookupBillEntity:

    def test_should_create_instance(self):
        testsdk = DingconnectSDK.test(None, None)
        ent = testsdk.LookupBill(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _lookup_bill_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "lookup_bill." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DINGCONNECT_TEST_LOOKUP_BILL_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        lookup_bill_ref01_ent = client.LookupBill(None)
        lookup_bill_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.lookup_bill"), "lookup_bill_ref01"))

        lookup_bill_ref01_data = helpers.to_map(runner.entity_data(lookup_bill_ref01_ent.create(lookup_bill_ref01_data, None)))
        assert lookup_bill_ref01_data is not None



def _lookup_bill_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/lookup_bill/LookupBillTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "DINGCONNECT_TEST_LOOKUP_BILL_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("DINGCONNECT_TEST_LOOKUP_BILL_ENTID"))
